...
namespace ArchitectureVisualizer
{
    public class ArchitectureVisualizerWindow : EditorWindow
    {
        ...
        private void UpdateEventTracking()
        {
            ...
            foreach (var path in EventTrackingManager.TrackingPaths)
            {
                // Контейнер для всего пути
                var pathRoot = new VisualElement();
                pathRoot.AddToClassList("et-path-container");
                eventTrackingContainer.Add(pathRoot);
                ...
                expandButton.text = path.isExpanded ? "▼" : "►";
                expandButton.style.width = 22;
                expandButton.style.height = 18;
                expandButton.style.marginRight = 4;
                expandButton.style.unityFontStyleAndWeight = FontStyle.Bold;
                headerRow.Add(expandButton);

                // Красная точка если активен
                if (path.isTracking)
                {
                    var indicator = new VisualElement();
                    indicator.AddToClassList("et-path-indicator");
                    headerRow.Add(indicator);
                }

                // Название пути
                var nameLabel = new Label(path.pathName);
                nameLabel.style.unityFontStyleAndWeight = FontStyle.Bold;
                nameLabel.style.fontSize = 13;
                nameLabel.style.marginRight = 6;
                if (path.isTracking)
                    nameLabel.style.color = new Color(0.9f, 0.2f, 0.2f);
                headerRow.Add(nameLabel);

                // --- Содержимое пути ---
                if (path.isExpanded)
                {
                    // Всё содержимое, что раньше было внутри Foldout, теперь добавляем сюда:
                            ...
                            foreach (var instance in step.trackedInstances)
                            {
                                if (instance is MultiVarTrackedInstance multiInst)
                                {
                                    ...
                                    foreach (var varName in variables)
                                    {
                                        ...
                                        {
                                            valueLabel = new Label($"{gameObjectName} [{varName} ({typeStr})]: {val}");
                                        }
                                        else
                                        {
                                            valueLabel = new Label($"{gameObjectName} [{varName} ({typeStr})]: {val}");
        ...
        private void ToggleTracking(TrackingPath path)
        {
            ...
            // --- Костыль: перекрашиваем заголовок Foldout напрямую ---
            foreach (var foldout in eventTrackingContainer.Children().OfType<Foldout>())
            {
                if (foldout.text == path.pathName)
                {
                    var label = foldout.Q<Label>("unity-foldout__text");
                    if (label != null)
                    {
                        if (path.isTracking)
                            label.style.color = new Color(0.9f, 0.2f, 0.2f);
                        else
