...
### Последние улучшения
- **Выбор нескольких переменных:** Теперь в одном шаге можно отслеживать не одну, а сразу несколько переменных из выбранного скрипта.
- **Поддержка обычных C# классов:** Реализована возможность отслеживать переменные в классах, не наследующихся от `MonoBehaviour`. Плагин позволяет указать "хост-компонент", в котором находится экземпляр нужного класса.
- **Новый интерфейс выбора скриптов:** Вместо выпадающего списка теперь используется отдельное окно с поиском, что значительно упрощает навигацию в больших проектах.
- **Улучшения UI:** Окна добавления шагов/путей теперь имеют адекватный стартовый размер, а текст в элементах управления выровнен для лучшей читаемости.

### Следующие шаги
- **Улучшение интерфейса Event Tracking:** Дальнейшая работа над основной вкладкой для более удобного отображения и управления путями и шагами.
