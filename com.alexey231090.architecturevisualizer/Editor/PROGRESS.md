...
- Улучшен UI редактирования шага: кнопки "Удалить", "Добавить переменную", "Сохранить" теперь имеют приятные светлые цвета (красный, жёлтый, зелёный).
- Исправлен баг: переменные для редактирования шага теперь корректно отображаются для всех скриптов.
- Исправлен баг: нельзя было добавить/удалить переменные шага, если они есть в скрипте.
- Исправлен баг: кнопки теперь перекрашиваются полностью, а не только текст.
- Исправлен баг: при отсутствии переменных для скрипта выводится корректное сообщение.

## Последние изменения

- **[НОВОЕ]** Полная миграция системы хранения путей и шагов Event Tracking на JSON-файлы:
  - Основной файл: `ProjectSettings/ArchitectureVisualizer.paths.json` (входит в git).
  - AI-файл: `ProjectSettings/ArchitectureVisualizer.ai.json` (для предложений от ИИ и коллег).
- **[НОВОЕ]** Интеграция с ИИ:
  - В AI-файле есть подробная инструкция и пример для генерации новых путей.
  - В плагине реализовано отображение AI-путей с возможностью принять их одной кнопкой.
  - После принятия путь переносится в основной файл и исчезает из AI-файла.
- **[НОВОЕ]** EditorPrefs полностью удалён, все данные теперь надёжно хранятся в проекте.
- **[НОВОЕ]** Улучшен UI/UX:
  - Пути из AI-файла выделяются цветом и помечаются как "AI Suggestion".
  - Кнопка "✔ Принять" для быстрого переноса пути.
- **[НОВОЕ]** Документация и техническое описание полностью актуализированы.
- **[НОВОЕ]** При первой установке плагина автоматически создаётся текстовая инструкция ArchitectureVisualizer.ai.instructions.txt для AI.
- **[НОВОЕ]** Для подгрузки новых AI-путей не требуется перезапуск Unity — достаточно нажать "Analyze Project".
- **[НОВОЕ]** Вся документация и примеры по формату AI json теперь доступны в отдельном .txt-файле рядом с рабочим json.
- **Планы:**  
  - Дальнейшее развитие AI-интеграции (например, автоматическое обновление при изменении AI-файла).
  - Улучшение пользовательского опыта и расширение возможностей Event Tracking.

## Следующие шаги

- Дальнейшее улучшение интерфейса Event Tracking.
- Оптимизация производительности.
- Улучшение документации и примеров использования.

## Важно

> Все устаревшие функции (Tables, Script Details) не развиваются и будут удалены только по отдельной команде.

## Новый прогресс (Event Tracking и заметки)
- Основной фокус — система Event Tracking для отладки и заметок
- Добавление, редактирование, удаление цепочек (paths) и шагов (steps)
- Поддержка вложенных переменных, коллекций, массивов, словарей
- Многострочные комментарии для шагов и цепочек
- Поддержка кириллицы (работает "из коробки", баг с вводом комментариев полностью решён, кнопка "Рус" больше не нужна)
- Подсветка изменений (fade/highlight) при изменении значений
- Реальное время обновления значений переменных
...
- Исправлена ошибка CS0246 (TMPro не найден): добавлена зависимость в asmdef
- Исправлен баг с MissingReferenceException при удалении отслеживаемого объекта (теперь отображается [Destroyed] и ошибок нет)
- Улучшен UX: переменные, которые ни разу не менялись, остаются серыми, а изменённые становятся белыми — удобно для анализа
- Баг с кириллицей и многострочными комментариями полностью решён, всё работает без костылей и без кнопки "Рус"

### Следующие шаги
- Дальнейшее улучшение интерфейса Event Tracking
- Отображение типов отслеживаемых переменных
- Оптимизация производительности, расширение поддержки компонентов, улучшение документации

## Прошлый прогресс (до смены направления)
- Реализован анализ зависимостей между скриптами
- Таблицы зависимостей (Tables)
- Вкладка Script Details (детальный анализ скриптов)
- Отображение структуры проекта
- Группировка скриптов по папкам
- Поддержка вложенных типов, массивов, коллекций
- Базовая интеграция с Unity Editor
